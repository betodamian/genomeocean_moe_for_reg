...
- Ground-truth sources (experimental, never software-predicted):
  ...
  **Phylum coverage for RBS = 3 phyla** (Gammaproteobacteria: E. coli; Actinobacteria: M. tuberculosis; Firmicutes: B. subtilis + S. aureus). *P. aeruginosa* (also Gammaproteobacteria) contributes **no experimental** RBS truth. Leave-one-phylum-out (P5) for RBS therefore spans 3 phyla; **S. aureus, with its divergent extended-SD motif, is the strongest cross-genome generalization test** (genuinely novel SD sequence, not just novel flanks).
...
| **Shine-Dalgarno RBS** | Curated DB `data/rbs_database/` (8 Ribo-seq/dRNA-seq sources); SD vs **UNSD-leadered** primary decoy (§5c) | ~11k SD positives; ~2.5–3.5k UNSD negatives; ~1.8k leaderless (secondary). **3 phyla** (Gamma/Actino/Firmicutes) | **Yes via UNSD** (GC-clean, every Ribo-seq organism). *Leaderless* negative is GC-confounded (MTB 65.5%) → secondary only. S. aureus extended-SD = strongest test |
